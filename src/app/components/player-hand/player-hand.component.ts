...
import {
    ...
    CardBtnAction,
} from '../card-actions/card-actions.component';
...
export class PlayerHandComponent {
        ...
        [
            {
                ...
                icon: 'arrow_right',
                hide$: combineLatest([
                    this.gameFacade.getCurrentPhase$().pipe(
                        takeUntilDestroyed(this.destroyRef),
                        map(this.isActionPhase),
                        map(isActionPhase => isActionPhase)
                    ),
                    this.gameFacade.getCard$(cardId).pipe(
                        takeUntilDestroyed(this.destroyRef),
                        mergeMap(gameCard =>
                            this.gameFacade.playFromHandIsAllowed$(gameCard!)
                        )
                    ),
                ]).pipe(
                    map(([actionPhaseAllowed, playFromHandAllowed]) => {
            ...
            },
            {
                ...
                hide$: combineLatest([
                    ...
                    map(([archiveAlreadyHasCard, gamePhase]) => {
                        return (
                            archiveAlreadyHasCard ||
                            !this.isActionPhase(gamePhase)
                        );
                    })
                ),
            },
    ...
    private isActionPhase(phase: GamePhase) {
        return phase === 'action';
    }
}

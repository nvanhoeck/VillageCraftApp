...
export class CardActionsComponent implements OnInit {
    ...
    ngOnInit() {
        this.eligibleActions$ = this.gameFacade
            .getAllowedActions$(this.card!.id, this.gameSpace!)
            .pipe(
                map(actions => {
                    return [
                        ...this.defaultActions,
                        ...actions.map(action =>
                            this.mapCardActionToButtonAction(action)
                        ),
                    ];
                })
            );
    ...
    handleClick(actionType: Trigger) {
        switch (actionType) {
            ...
            case 'exhaust':
                ...
                break;
            case 'archive':
                this.gameFacade.playCardFromTo(
                    this.gameSpace!,
                    'ARCHIVE',
                    this.card!.id
                );

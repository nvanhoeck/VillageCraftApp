...
@Component({
    ...
    imports: [
        ...
        PlayerDeckComponent,
        ArchiveComponent,
        CitizenLaneComponent,
        BuildingLaneComponent,
        DragScrollComponent,
...
export class GameComponent {
    ...
    game$ = this.gameSetupFacade
        .getGame$()
        .pipe(takeUntilDestroyed(this.destroyRef));
    playersIds$ = this.gameSetupFacade
        .getPlayerIds$()
        .pipe(takeUntilDestroyed(this.destroyRef));
    gamePhase$ = this.game$.pipe(
    ...
    constructor(
        private readonly gameSetupFacade: GameSetupFacadeService,
        private readonly gamePhaseFacade: GamePhaseFacadeService
    ) {}

    handleMulliganCardClicked(cardId: string) {
        const cardAlreadyFound = this.mulliganCardsSelected.find(
            id => id === cardId
        );
        if (cardAlreadyFound) {
            this.mulliganCardsSelected.splice(
                this.mulliganCardsSelected.indexOf(cardAlreadyFound),
                1
            );
        } else {

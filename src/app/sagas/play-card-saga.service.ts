...
export class PlayCardSagaService implements EventHandler {
  ...
  playFromHandTo(to: "CITIZEN_LANE" | "BUILDING_LANE", cardId: string, playerId: string, gameId: string) {
    ...
    this.cardId = cardId
    if (to === 'CITIZEN_LANE') {
      this.eventBus.on(new DisplayCitizenLanePlaySlotsEvent({playerId, gameId}))
    } else {
      this.eventBus.on(new DisplayBuildingLanePlaySlotsEvent({playerId, gameId}))
    }
  }
    ...
    switch (event.type) {
      case 'DisplayCitizenLanePlaySlots':
        this.errorMessageService.publish({
          level: 'INFO', message: `Pick a spot to play your citizen card`,
          topic: "GAME-INFO"
        });
        break;
      case 'DisplayBuildingLanePlaySlots':
        this.errorMessageService.publish({
          level: 'INFO', message: `Pick a spot to play your building card`,
          topic: "GAME-INFO"
        });
        break;
      case 'CitizenLaneSlotChosen':
        this.commandBus.on(new PlayCardFromToCommand({
          from: 'HAND',
          to: 'CITIZEN_LANE',
        ...
        this.eventBus.on(new HideCitizenLanePlaySlotsEvent({playerId: this.playerId!, gameId: this.gameId!}))
      ...
      case 'BuildingLaneSlotChosen':
        ...
        this.eventBus.on(new HideBuildingLanePlaySlotsEvent({playerId: this.playerId!, gameId: this.gameId!}))
        this.cardId = undefined
        break;
      default:
        this.errorMessageService.publish({
          level: 'ERROR', message: `Could not handle event ${event.type} in PlayCardSaga`,

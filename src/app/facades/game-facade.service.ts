...
import {
    ...
    GamePhase,
    Trigger,
    TRIGGER_FROM_GAME_SPACE,
} from '../query/model/game-card-vo';
...
export class GameFacadeService {
    ...
    constructor(
        private gameSetupFacade: GameSetupFacadeService,
    ...
    getPlayerBuildingLane$() {
        return this.getPlayerBuildingLaneUseCase.getPlayerBuildingLane$(
            this.playerId
        );
    }

    getPlayerCitizenLane$() {
        return this.getPlayerCitizenLaneUseCase.getPlayerCitizenLane$(
            this.playerId
        );
    }

    getPlayerDiscardPile$() {
        return this.getPlayerDiscardPileUseCaseService.getPlayerDiscardPile$(
            this.playerId
        );
    }

    getPlayerGraveyard$() {
        return this.getPlayerGraveyardUseCaseService.getPlayerGraveyard$(
            this.playerId
        );
    }

    getPlayerBanishment$() {
        return this.getPlayerBanishmentUseCaseService.getPlayerBanishment$(
            this.playerId
        );
    }

    playCardFromTo(from: GameSpace, to: GameSpace, cardId: string) {
        this.playCardFromUseCaseService.playCardFromTo(
            from,
            to,
            cardId,
            this.playerId,
    ...
    showSlots$(slotType: 'citizen' | 'building') {
        if (slotType === 'citizen') {
            return this.shouldShowSlotsForPlayerUseCaseService.shouldShowCitizenSlotsForPlayer$(
                this.playerId,
                this.gameId!
            );
        } else {
            return this.shouldShowSlotsForPlayerUseCaseService.shouldShowBuildingSlotsForPlayer$(
                this.playerId,
                this.gameId!
            );
        }
    ...
    citizenSlotSelected(index: number) {
        this.playerSelectSlotsForCardUseCaseService.selectCitizenLaneSlot(
            index,
            this.gameId!,
            this.playerId
        );
    ...
    buildingSlotSelected(index: number) {
        this.playerSelectSlotsForCardUseCaseService.selectBuilingLaneSlot(
            index,
            this.gameId!,
            this.playerId
        );
    ...
    getWood$() {
        return this.getPlayerResourceUseCaseService
            .getPlayerResources$(this.gameId!, this.playerId)
            .pipe(map(resources => resources.wood));
    }

    getFood$() {
        return this.getPlayerResourceUseCaseService
            .getPlayerResources$(this.gameId!, this.playerId)
            .pipe(map(resources => resources.food));
    }
    ...
    exhaustCard(gameSpace: GameSpace, cardId: string) {
        this.triggerCardEffectUseCaseService.exhaustCard(
            gameSpace,
            cardId,
    ...
    getCard$(id: string, gameSpace?: GameSpace) {
        switch (gameSpace) {
            case 'BUILDING_LANE':
                return this.getPlayerBuildingLane$().pipe(
                    map(lane => lane.findCardInLane(id))
                );
            case 'CITIZEN_LANE':
                return this.getPlayerCitizenLane$().pipe(
                    map(lane => lane.findCardInLane(id))
                );
            case 'SETTLEMENT':
                return this.getSettlement$();
            case 'HAND':
                return this.getPlayerHand$().pipe(
                    map(hand => hand.find(card => card.id === id))
                );
            //TODO improve
            case undefined:
                return this.getPlayerHand$().pipe(
            ...
            default:
                throw new Error(
                    `case ${gameSpace} not defined to fetch card, built code for it`
                );
        }
    }

    gainWood(gameSpace: GameSpace, cardId: string) {
        this.triggerCardEffectUseCaseService.gainWood(
            gameSpace,
            cardId,
            this.gamePhaseFacade.getGamePhase(this.gameId!),
            this.gameId!,
    ...
    gainFood(gameSpace: GameSpace, cardId: string) {
        this.triggerCardEffectUseCaseService.gainFood(
            gameSpace,
            cardId,
            this.gamePhaseFacade.getGamePhase(this.gameId!),
            this.gameId!,
        ...
        if (gameCardVO.cardType === 'building') {
            return this.getPlayerResourceUseCaseService
                ...
                .pipe(
                    ...
                    map(resources => {
                        return (
                            resources.wood >= (gameCardVO.cost?.wood ?? 0) &&
                            resources.food >= (gameCardVO.cost?.food ?? 0)
                        );
                    })
                );
        } else return of(true);
            ...
            map(([card, gamePhase]) => {
                if (card) {
                    return card.actions.filter(action =>
                        this.isActionAllowed(action, card, gameSpace, gamePhase)
                    );
                } else {
                    return [];
                }
    ...
    getLocationsByLane$(row: number) {
        return this.getLocationsByLaneUseCaseService.getLocationsByLane$(
            this.gameId!,
            row
        );
    }

    private triggerAreAllowedFromGameSpace(
        trigger: Trigger,
        gameSpace: GameSpace
    ) {
    ...
    private isActionAllowed(
        ...
        gameSpace: GameSpace,
        gamePhase: GamePhase
        //playerResources: { wood: number; food: number }
    ) {
        switch (action.trigger) {
            ...
            case 'exhaust':
                return (
                    !card.exhausted &&
                    ['BUILDING_LANE', 'CITIZEN_LANE'].includes(gameSpace) &&
                    gamePhase === 'action'
                );
            case 'deploy':
            ...
            case 'banish':
                return (
                    ['BUILDING_LANE', 'CITIZEN_LANE', 'HAND'].includes(
                        gameSpace
                    ) && gamePhase === 'action'
                );
            case 'claim':

...
export class MessagesAdapterService {
    private applicationMessageHandlers: {
        [key: string]: ApplicationMessageListener[];
    } = {};

    public publish(applicationMessage: ApplicationMessage) {
        if (this.applicationMessageHandlers[applicationMessage.topic]) {
            this.applicationMessageHandlers[applicationMessage.topic].forEach(
                listener => {
            ...
            throw new Error(
                'No listener found for topic ' + applicationMessage.topic
            );
        }
    }

    public register(
        listener: ApplicationMessageListener,
        topic: ApplicationMessageTopic
    ) {
        if (this.applicationMessageHandlers[topic]) {
            this.applicationMessageHandlers[topic] = [
                ...this.applicationMessageHandlers[topic],
                listener,
            ];
        } else {

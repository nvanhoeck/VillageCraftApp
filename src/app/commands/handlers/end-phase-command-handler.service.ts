    ...
    execute(cmd: Command): void {
        if (isEndPhaseCommand(cmd)) {
            ...
            const player = game.players.find(
                p => p.id === cmd.payload.playerId
            )!;
            this.eventBus.on(
                new PlayerPhaseStartedEvent({
                    gameId: game.id,
                    playerId: cmd.payload.playerId,
                    nextPhase: player.findInPhase(),
                })
            );
            if (hasGamePhaseEnded) {
                this.eventBus.on(
                    new GamePhaseStartedEvent({
                        gameId: game.id,
            ...
            this.errorMessageService.publish({
                level: 'ERROR',
                message: `Wrong command sent for EndPhaseCommandHandlerService for ${cmd.type}`,

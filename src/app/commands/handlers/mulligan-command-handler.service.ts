...
export class MulliganCommandHandlerService implements CommandHandler {
  ...
  execute(cmd: Command): void {
    if (isMulliganCommand(cmd)) {
      if (cmd.payload.cardIds.length > 0) {
        ...
        if (!player) {
          this.errorMessageService.publish({
            level: 'ERROR', message: `Player not foun in MulliganCommandHandlerService for ${cmd.payload.playerId}`,
      ...
      this.errorMessageService.publish({
        level: 'ERROR', message: `Wrong command sent for MulliganCommandHandlerService for ${cmd.type}`,
        topic: "APPLICATION-ERROR"
      })
    }
  }

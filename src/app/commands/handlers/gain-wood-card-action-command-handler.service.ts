        ...
        if (isGainWoodCardActionCardCommand(cmd)) {
            ...
            if (!player) {
                this.errorMessageService.publish({
                    level: 'ERROR',
                    message: `Player not found in GainWoodCardActionCommandHandlerService for ${cmd.payload.playerId}`,
                    topic: 'APPLICATION-ERROR',
                });
            } else {
                const amount = player.gainWoodFromCard(
                    cmd.payload.cardId,

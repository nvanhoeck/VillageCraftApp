    ...
    if (isLoadPlayerBaseDecksCommand(cmd)) {
      ...
      this.deckAdapterService.loadBaseDeck().pipe(first()).subscribe((baseCards) => {
        game.players.forEach((player) => {
          ...
          this.eventBus.on(new PlayerBaseDecksLoadedEvent({player: player}))
        })
      })
    } else {
      this.errorMessageService.publish({
        level: 'ERROR', message: `Wrong command sent for LoadPlayerBaseDecksCommandHandlerService for ${cmd.type}`,

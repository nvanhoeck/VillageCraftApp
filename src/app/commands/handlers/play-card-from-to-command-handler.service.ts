...
import {
    isPlayCardFromToCommand,
    PlayCardFromToCommand,
} from '../model/play-card-from-to-command';
...
export class PlayCardFromToCommandHandlerService implements CommandHandler {
    ...
    execute(cmd: Command): void {
        if (isPlayCardFromToCommand(cmd)) {
            ...
            this.errorMessageService.publish({
                level: 'ERROR',
                message: `Wrong command sent for PlayCardFromToCommandHandlerService for ${cmd.type}`,
                topic: 'APPLICATION-ERROR',
            });
        }
    }
    ...
    private handlePlayCardFromHandToArchive(
        game: Game,
        cmd: PlayCardFromToCommand
    ) {
    ...
    private handlePlayCardFromHandToLane(
        game: Game,
        cmd: PlayCardFromToCommand
    ) {
        const foundPlayer = game.players.find(
        ...
        if (cmd.payload.to === 'CITIZEN_LANE') {
            foundPlayer.playCardFromHandToCitizenLaneAtSlot(
                cmd.payload.cardId,
                cmd.payload.index
            );
            const hand = foundPlayer?.findHand();
            ...
            this.eventBus.on(
                new CardPlayedToCitizenLaneEvent({
            ...
            foundPlayer.playCardFromHandToBuildingLaneAtSlot(
                cmd.payload.cardId,
                cmd.payload.index
            );
            const hand = foundPlayer?.findHand();
            ...
            this.eventBus.on(
                new CardPlayedToBuildingLaneEvent({

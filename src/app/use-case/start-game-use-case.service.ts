...
export class StartGameUseCaseService {
    ...
    public startPlayerVsPcGame(gameId: string, playerId: string) {
        ...
        //Setup game
        this.commandBus.on(
            new SetupGameCommand({ id: gameId, gameType: 'PVC' })
        );
        //Setup Locations
        this.commandBus.on(new LoadLocationsCommand({ gameId }));
        //Setup player
        this.commandBus.on(
            new SetupPlayerCommand({
                gameId,
                playerId: playerIdHuman,
                playerType: 'HUMAN',
            })
        );
        //Setup pc
        ...
        this.commandBus.on(
            new LoadPlayerBaseDecksCommand({
                gameId,
                playerIds: [playerIdPc, playerIdHuman],
            })
        );

...
export class GameProjectionService implements EventHandler {
    ...
    shouldShowCitizenSlotFor$(playerId: string, gameId: string) {
        return this.games[gameId].pipe(
            map(
                game =>
                    game.players
                        .find(playerInfo => playerInfo?.id === playerId)
                        ?.shouldShowCitizenSlot() ?? false
            )
        );
    }
    ...
    getPlayerResources$(gameId: string, playerId: string) {
        return this.players[playerId].pipe(
            map(player => player.getResources())
        );
    }
    ...
    private handleGameCreatedEvent(event: GameCreatedEvent) {
        this.games[event.payload.id] = new BehaviorSubject<Game>(
            new Game(event.payload.id, event.payload.type)
        );
    }

    private handlePlayerBaseDecksLoadedEvent(
        event: PlayerBaseDecksLoadedEvent
    ) {
        this.players[event.payload.player.id]
            .pipe(take(1))
            .subscribe(player => {
                player.updateSettlement(event.payload.player.findSettlement());
                player.updateCitizenLane(
                    event.payload.player.findCitizenLane()
                );
                player.updateDeck(event.payload.player.findDeck());
                player.updateDiscardPile(
                    event.payload.player.findDiscardPile()
                );
                ...
                player.updateBuildingLane(
                    event.payload.player.findBuildingLane()
                );
    ...
    private handlePlayerCreatedEvent(event: PlayerCreatedEvent) {
        this.players[event.payload.id] = new BehaviorSubject<Player>(
            new Player(event.payload.id, event.payload.playerType)
        );
    ...
    private handleDisplayCitizenLanePlaySlots(
        event: DisplayCitizenLanePlaySlotsEvent
    ) {
        this.games[event.payload.gameId].pipe(take(1)).subscribe(game => {
            game.players
                .find(player => player.id === event.payload.playerId)
                ?.showCitizenSlot();
            this.games[event.payload.gameId].next(game);
        });
    }
    ...
    private handleCardPlayedToBuildingLane(
        event: CardPlayedToBuildingLaneEvent
    ) {
        this.players[event.payload.playerId].pipe(take(1)).subscribe(player => {
    ...
    private handleHideBuildingLanePlaySlots(
        event: HideBuildingLanePlaySlotsEvent
    ) {
        this.games[event.payload.gameId].pipe(take(1)).subscribe(game => {
            game.players
                .find(player => player.id === event.payload.playerId)
    ...
    private handleHideCitizenLanePlaySlots(
        event: HideCitizenLanePlaySlotsEvent
    ) {
        this.games[event.payload.gameId].pipe(take(1)).subscribe(game => {
            game.players
                .find(player => player.id === event.payload.playerId)
    ...
    private handlePlayerGainedWood(event: PlayerGainedWoodEvent) {
        this.players[event.payload.playerId].pipe(take(1)).subscribe(player => {

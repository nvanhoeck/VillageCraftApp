...
export class Game {
    ...
    updateLocations(locationLanes: LocationLanes) {
        this._locations.one = [
            ...locationLanes.one.map(locationC =>
                this.mapLocationLaneDomainToVO(locationC)
            ),
        ];
        this._locations.two = [
            ...locationLanes.two.map(locationC =>
                this.mapLocationLaneDomainToVO(locationC)
            ),
        ];
        this._locations.three = [
            ...locationLanes.three.map(locationC =>
                this.mapLocationLaneDomainToVO(locationC)
            ),
        ];
    }
    ...
    findLocationsInLane(row: number): LocationCardVO[] {
        switch (row) {
            case 1:
                return this._locations.one;
            case 2:
                return this._locations.two;
            case 3:
                return this._locations.three;
            default:
                return [];
        }
    }

...
export class Player {
    ...
    updateCitizenLane(lane: DomainLane) {
        this._citizenLane = new Lane('citizen');
        lane.findCardsInLane()
            .map(this.mapDomainCardToVO)
            .forEach(c => this._citizenLane.addCard(c));
    }

    updateDeck(
        gameCards: (GameCitizenCard | GameBuildingCard | GameEventCard)[]
    ) {
    ...
    updateArchive(
        archive: GameCitizenCard | GameBuildingCard | GameEventCard | undefined
    ) {
    ...
    updateBuildingLane(lane: DomainLane) {
        ...
        lane.findCardsInLane()
            .map(this.mapDomainCardToVO)
            .forEach(c => this._buildingLane.addCard(c));
    }
    ...
    private mapDomainCardToVO(
        card: GameCitizenCard | GameBuildingCard | GameEventCard
    ) {
        const gameCard = {
            id: card.id,
            cardId: card.cardId,
        ...
        if (isBuildingCard(card)) {
            return {
                ...
                constructionProgress: card.constructionProgress,
                cost: card.cost,
            };
        } else {
            return gameCard;
